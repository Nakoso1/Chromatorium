...
| 機能 |できること| 理論・手法 |
| 色の選択・数値表示 | 画像上の任意の色を読み取り、複数の表色系で比較する | sRGBの線形化、Oklab / OkLCh、CIELAB、HSL / HSV。色差の表示には CIEDE2000 (ΔE00) を用います。 |
| 色相環・マンセル表示 | RGB、RYB、マンセルの色相環を比較し、マンセルHVCを立体表示する | RGB / RYB / マンセルの異なる色相配置の比較。マンセル値はOklabを内部計算に用いた近似であり、公式マンセルデータベースとの厳密な照合値ではありません。 |
| ヒストグラム・色覚シミュレーション | 輝度・RGB分布を確認し、P型・D型・T型色覚の見え方を比較する | 相対輝度、RGBヒストグラム、Machado et al. の色覚異常シミュレーション行列を用いた近似表示。医学的な診断を目的とするものではありません。 |
| 明度・階調・明瞭度の調整 | グレースケール化、階調制限、ディザ、局所的な明瞭度調整を行う | 線形光での輝度処理、量子化とディザリング、ぼかしとの差分を使う中間周波数の局所コントラスト処理。 |
| 色相・ガマットマスク | 特定の色相・彩度領域だけを観察・補正し、画像の実効ガマットを抽出する | OkLCh上での距離・角度によるマスク、k-meansクラスタリング、凸包（Andrew's monotone chain）による色域輪郭の抽出。 |
| 配色・パレット抽出 | 主要色を抽出し、補色などの配色ルールを試す | k-meansによる代表色抽出、OkLChの色相角に基づく補色・類似色・分裂補色などの幾何学的な配色規則。 |
| 顔料混色・レシピ | デジタルの重ね塗りと顔料の減法混色を比較し、混色段階や近似レシピを確認する | Kubelka–Munk理論に基づく減法混色の近似、CMY / RGBの理想混色との比較、ΔE00による近似誤差の表示。実在する絵具の配合結果を保証するものではありません。 |
